...
const Analytics: React.FC = () => {
  ...
  const getSubjectPerformance = () => {
    const subjects: { [key: string]: { correct: number; total: number; name: string } } = {};
    
    state.performance.forEach(item => {
      if (!subjects[item.subjectId]) {
        subjects[item.subjectId] = { correct: 0, total: 0, name: item.subjectId };
      }
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{state.totalAttempted}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{state.overallAccuracy.toFixed(1)}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{state.currentStreak}</p>
              </div>
            </div>
          </div>
              ...
              <div className="p-6">
                {subjectPerformance.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-center py-8">
                    No data available yet. Start practicing to see your performance!
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="p-6">
                {state.performance.length > 0 ? (
                  <div className="space-y-3">
                    {state.performance.slice(-5).reverse().map((item, index) => (
                      <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                        <div className={`w-3 h-3 rounded-full ${item.correct ? 'bg-green-500' : 'bg-red-500'}`} />
                        <div className="flex-1">
                          <p className="text-sm text-gray-600">
                            {new Date(item.timestamp).toLocaleTimeString()}
                          </p>
                          <p className="text-xs text-gray-500 capitalize">
                            {item.subjectId} • {item.correct ? 'Correct' : 'Incorrect'}
                          </p>
                        </div>
                      </div>
                    ))}
